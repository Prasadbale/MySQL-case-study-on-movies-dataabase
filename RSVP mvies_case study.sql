...
FROM
    director_mapping;						-- Number of rows in director_mapping table : 3867


SELECT 
    COUNT(*) AS total_rows_genre
FROM
    genre;									-- Number of rows in genre table : 14662


SELECT 
    COUNT(*) AS total_rows_movie
FROM
    movie; 									-- Number of rows in movie table : 7997


SELECT 
    COUNT(*) AS total_rows_names
FROM
    names;									-- Number of rows in names table : 25735


SELECT 
    COUNT(*) AS total_rows_ratings
FROM
    ratings; 								-- Number of rows in ratings table : 7997


SELECT 
    COUNT(*) AS total_rows_role_mapping
FROM
    role_mapping;							-- Number of rows in role_mapping table : 15615
...
-- 4 fields in movie table have null values i.e, country, World_wide_gross_income, Languages and Production_company
...
SELECT 
    COUNT(title) AS number_of_movies_USA_India,
    year
FROM
    movie
WHERE
...
-- Number of movies produced in USA or India in 2019 : 1059
...
WITH movie_genre AS								-- CTE for the number of genres for a particular movie 
(
	SELECT 
		movie_id,
...
(
	SELECT 
		genre,
		COUNT(movie_id) AS movie_count,
		DENSE_RANK() OVER (ORDER BY COUNT(movie_id) DESC) AS genre_rank			-- Ranking movie genre based on the number of movies made in the genre 
	FROM genre
	GROUP BY genre
)
SELECT 
...
WITH movie_ranking AS 								-- CTE for movie ranking 
(
	SELECT
		M.title,
		R.avg_rating,
		DENSE_RANK() OVER(ORDER BY R.avg_rating DESC) AS movie_rank				-- Ranking movies based on the average rating
	FROM	movie as M
			INNER JOIN
...
WITH top_production_comp AS 						 -- CTE for ranking top production companies with number of movies rated more than 8
(
	SELECT
...
WITH top_3_genre AS					-- CTE forranking genre based on the numver of movies made with average rating more than 8 
(
	SELECT 
		G.genre,
		COUNT(G.movie_id)
...
top_directors AS 					-- CTE for ranking directors based on number of movies made in top 3 genre
(
	SELECT 
		N.name,
...
WITH top_movies AS   					 	-- CTE for ranking movies with median rating > 8
(
SELECT 
    movie_id
FROM
...
WITH top_produciton_house AS 						-- CTE for ranking production houses based on total votes recieved
(
	SELECT 
		M.production_company,
...
SELECT 														-- Ranking actors based on their average rating 
	N.name AS actor_name,
...
SELECT 																-- Ranking actress based on their average rating
...
SELECT									-- Classifying movies as Superhit, hit, One-time-watch and Flop based on average rating
    M.title,
    R.avg_rating,
    CASE
...
WITH duration_summary AS				-- Calculating Running total and moving average of movie duration for each genre
(
	SELECT 
		G.genre,
		ROUND(AVG(M.duration),2) AS avg_duration
...
WITH top_3_genres AS								-- CTE for ranking genre based on number of movies made
(
	 SELECT 
		genre,
		COUNT(movie_id) AS movie_count
...
top_gross AS										-- CTE for ranking movies based on gross world wide income
(
	SELECT 
		G.genre,
		M.year,
...
SELECT 									-- Ranking Production companies based on number of multilingual movies made
	M.production_company,
    COUNT(M.id) AS movie_count,
...
WITH top_actress  AS					 -- Ranking actress based on average rating
(
	SELECT 
		N.name AS actress_name,
		SUM(R.total_votes) AS total_votes,
